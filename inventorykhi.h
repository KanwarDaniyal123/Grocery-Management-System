...
public:
	...
	void writei()
	{
		ofstream myFile("inventorykhi.bin", ios::binary | ios::app);
	...
	void dele()
	{
		...
		cin >> str1;
		inventorykhi temp;
		fstream myFile("inventorykhi.bin", ios::in | ios::out | ios::binary);
		...
		myFile.close();
		myFile_temp.close();
		remove("inventorykhi.bin");
		rename("temp.bin", "inventorykhi.bin");
	...
	void retrieve()
	{
		cout << endl << "\t\tWELCOME TO INVENTORY OF KARACHI";
		cout << left << "\n" << "NAME" << "\t\t\t" << "PRICE PER UNIT" << "\t\t" << "QUANTITY";
		inventorykhi temp;
		ifstream myFile("inventorykhi.bin", ios::binary);
