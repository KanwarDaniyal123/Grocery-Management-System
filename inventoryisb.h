...
public:
	...
	void writei()
	{
		ofstream myFile("inventoryi.bin", ios::binary | ios::app);
	...
	void dele()
	{
		...
		cin >> str1;
		inventoryi temp;
		fstream myFile("inventoryi.bin", ios::in | ios::out | ios::binary);
		...
		myFile.close();
		myFile_temp.close();
		remove("inventoryi.bin");
		rename("temp.bin", "inventoryi.bin");
	...
	void search()
	{
		...
		if (found == 0)
			...
			void retrieve()
			{
				cout << endl << "\t\tWELCOME TO INVENTORY OF ISLAMABAD";
				cout << left << "\n" << "NAME" << "\t\t\t" << "PRICE PER UNIT" << "\t\t" << "QUANTITY";
				inventoryi temp;
				ifstream myFile("inventoryi.bin", ios::binary);
